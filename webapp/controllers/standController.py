...
class StandController:
    ...
    def createStand(self, params):
        newStand = Stand(id=params[0], name=params[1], destructivePower=params[2], speed=params[3], range=params[4], \
            persistence=params[5], precision=params[6], developmentPotential=params[7], kind=params[8], classification=params[9], \
            sentient=params[10], abilities=params[11])
    ...
    def updateStand(self, id, params):
        ...
        if params.get('classification'):
            stand.classification = params['classification']
        if params.get('sentient'):
            stand.sentient = params['sentient']
